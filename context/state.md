...
| Dataset | IN PROGRESS | DATA-001 pinned the source revision and card-declared Apache-2.0 license; all three exact folders contain 30 usable original photos each. Ten per class are provisionally selected with an 8/2 identity-group split. The V1 TRAIN pilot ran on Kaggle T4; visual review found one potentially suitable output and two with material drift. Supervisor authorized a masked three-image V2 pilot, whose runner is prepared but has not yet executed on Kaggle. No official acceptance or directional pair exists. See [V1 review](../docs/data/DATA-001-pilot-review.md) and [V2 plan](../docs/data/DATA-001-pilot-v2.md). |
...
| Next approved task | DATA-001 PILOT V2 | Run only `CrewCut_1`, `BobHair_1`, and `LayeredHair_4` through the masked FLUX.2 Klein pilot, preserve masks/results/metadata, then stop for Supervisor/Project Lead visual review. No full generation or real LoRA training is authorized. |
