...
The [EXP-001 smoke test](../docs/experiments/EXP-001.md) verified Base image-edit inference, 20 finite-loss BF16 Edit-LoRA steps on one Kaggle T4, checkpoint serialization, and a fresh Base adapter load. FP16 produced NaN loss. The Supervisor's [TRAIN-001 run](../docs/experiments/TRAIN-001.md) then completed 250 finite-loss steps on DATA-001 with one conditional LoRA and saved final/intermediate adapters and optimizer state. Held-out hairstyle quality and improvement over Base remain UNKNOWN. Base-trained adapter compatibility with distilled Klein, multi-image refinement, and free deployment remain NEEDS VERIFICATION and are outside the current critical path.
