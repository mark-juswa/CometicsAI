...
EXP-001 harness: `notebooks/exp001_flux2_klein_kaggle_smoke.py`. Local verification: Python 3.11.5 compiled its syntax, and the Pillow generator produced three matching synthetic pairs in a temporary directory. Kaggle package versions, CUDA, training command success, and GPU runtime remain UNKNOWN until [EXP-001](../docs/experiments/EXP-001.md) runs on an actual Kaggle GPU. The script records the AI Toolkit commit and exact installed dependencies at run time; neither has yet been observed from Kaggle.
The first reached Kaggle CPU notebook showed `/usr/lib/python3.12` in its traceback and failed to find `nvidia-smi`; its accelerator menu had `None` selected. Exact Python patch version, PyTorch version, GPU hardware, CUDA, RAM, and disk were not reported. The harness now records a missing `nvidia-smi` instead of crashing before its environment report.
