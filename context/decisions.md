...
| DATA-001 V1/global FLUX.2 Klein Base editing for the remaining 27 selected identities; masked V2 optional fallback | CONFIRMED | Three-image V1 pilot VERIFIED; remaining 27 NOT STARTED | Supervisor's critical-path direction supersedes the earlier strict pilot/V2 prerequisite. Generate with the fixed three-class cycle, then apply the practical human-review standard to all 30. No output is auto-accepted. |
| One conditional Edit-LoRA for CrewCut, BobHair, and LayeredHair; evaluate at 250 steps and continue the same run toward 500 only if learning but underfit | CONFIRMED | Training mechanics VERIFIED only in EXP-001; real training NOT STARTED | Supervisor's current model and training direction. Same-run resume support is a preparation gap before any 500-step continuation. |
