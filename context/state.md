...
| Dataset | IN PROGRESS | DATA-001 pinned the source revision and card-declared Apache-2.0 license; all three exact folders contain 30 usable original photos each. Ten per class are provisionally selected with an 8/2 identity-group split. The three-image TRAIN pilot ran on Kaggle T4. Codex's visual review found one potentially suitable output and two with material drift; the approved 0–1-of-3 pilot rule stops further generation pending Supervisor/Project Lead methodology review. No official acceptance or directional pair exists. See [pilot review](../docs/data/DATA-001-pilot-review.md). |
...
| Next approved task | DATA-001 PILOT STOP | Supervisor/Project Lead reviews the three outputs and decides how to address the observed drift. No retry, full generation, or real LoRA training is authorized by this review. |
