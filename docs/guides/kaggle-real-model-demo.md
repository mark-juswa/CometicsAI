...
The first live bootstrap stopped at pip resolution because the original Hub cap `<1` conflicted with Diffusers 0.40.0's `>=1.23.0` requirement. The repository constraints now align with Diffusers 0.40.0 and the Kaggle-installed Transformers 5.0.0. Rerun the setup cell after pulling the corrected repository. If pip still stops, its last error lines appear in the bootstrap output and the full record remains in `dependency_install.log`. This dependency correction has not yet passed the live Kaggle rerun.
