...
During Base loading, the runner now prints a heartbeat every 60 seconds and a Python stack trace every 180 seconds. This was added after the Supervisor reported two long silent Base-load stalls with zero generated images. If loading stalls again, interrupt once and preserve the latest traceback and heartbeat output before another attempt; repeating the run without a diagnosis wastes the Kaggle session.
