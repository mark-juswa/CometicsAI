...
| Dataset | IN PROGRESS | DATA-001 has 30 selected originals with an 8/2 split per class and three completed V1/global pilot outputs from an earlier Kaggle session. In a fresh Supervisor session, `--plan-all` correctly listed 27 jobs, then `--all` stopped before download because `/kaggle/working/data001/` lacked the three V1 pilot artifacts; restore `data001_pilot.zip` with `--pilot-zip` and rerun. No official ACCEPT reviews, full generation, or directional pairs exist yet. Masked V2 is optional fallback only. See [active handoff](../docs/data/DATA-001-v1-bulk-handoff.md). |
| Training | VERIFIED SMOKE; REAL RUN NOT STARTED | FP16 failed with NaN loss. BF16 produced 20/20 finite losses, with observed optimizer steps and checkpoint serialization. A preliminary TRAIN-001 configuration exists, but the requested same-run continuation from 250 to 500 still needs preparation after DATA-001 finalization. No real hairstyle training has run. |
...
| Next approved task | DATA-001 V1 BULK | Supervisor runs the remaining 27 selected global-edit generations, reviews all 30, and returns the outputs/logs. Codex does not run long Kaggle jobs. Finalization still requires 30 explicit ACCEPT decisions and QA. TRAIN-001 execution waits for finalized data. |
