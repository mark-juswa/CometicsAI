...
The first live bootstrap stopped at pip resolution because the original Hub cap `<1` conflicted with Diffusers 0.40.0's `>=1.23.0` requirement. The repository constraints now align with Diffusers 0.40.0 and the Kaggle-installed Transformers 5.0.0. The next Kaggle run passed dependency setup. If pip stops in a future run, its last error lines appear in the bootstrap output and the full record remains in `dependency_install.log`.
The next live run passed dependency setup, FLUX import, adapter verification, and Base download. It stopped during LoRA loading because Kaggle had optional torchao 0.10.0, while PEFT requires at least 0.16.0 if torchao is installed. This FP16 CPU-offload runtime does not use torchao quantization. Bootstrap now removes only an installed torchao below 0.16.0 and saves that action to `torchao_remove.log`; it does not replace PyTorch. A live rerun is still required to verify server and tunnel readiness.
