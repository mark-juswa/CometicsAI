...
## First live web-app result

The Supervisor later authorized integration of this experimental checkpoint and reported a successful browser-to-local-API-to-Kaggle generation using the active GPU server. The submitted example shows the requested crew cut, while the scene changes to a gray gradient and the face is reconstructed. This verifies a working request path, not satisfactory preservation quality. The attached source image was 1024×1536 RGBA with 730,116 fully transparent pixels out of 1,572,864; about 48.4% of pixels had alpha below 250. Its top-left pixel was nearly transparent white `(255, 255, 255, 2)`. The returned example was 512×512 RGB. The apparent black backdrop in the source display is therefore largely transparency, not a black photographic background. Local API preprocessing converts the upload to RGB, and the global FLUX edit emits an opaque image. This explains the loss of transparency; the generated gray gradient also shows global scene reconstruction. An opaque source can test whether the scene drift persists independently of transparency. No additional GPU run or model change was made by Codex for this observation.
